...
function buildHtml(p: {
  ...
  const { firstName, lastName, sessionName, dateStr, associationName, displayCode, cartons, amount, ref, paiementId } = p
  ...
  const playBlock = `<tr><td style="padding:0 28px 24px;">
      <table width="100%" cellpadding="0" cellspacing="0" style="background:#0b1220;border-radius:8px;padding:16px 20px;">
        <tr>
          <td>
            <div style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.08em;color:rgba(255,255,255,.45);margin-bottom:6px;">Suivez votre carton en direct</div>
            ...
            <a href="${APP_URL}/play/${paiementId}" style="display:inline-block;background:#EF9F27;color:#2C1500;font-weight:700;font-size:12px;padding:8px 18px;border-radius:7px;text-decoration:none;">
...
function buildReminderHtml(p: {
  ...
  const { firstName, lastName, sessionName, dateStr, associationName, displayCode, cartons, ref, paiementId } = p
...
<head>
  <meta charset="UTF-8"/>
  <meta name="viewport" content="width=device-width,initial-scale=1.0"/>
  <title>C&apos;est demain — ${sessionName}</title>
</head>
<body style="margin:0;padding:0;background:#f1f5f9;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Arial,sans-serif;">
<table width="100%" cellpadding="0" cellspacing="0" style="background:#f1f5f9;padding:32px 16px;">
<tr><td align="center">
      ...
      <table width="100%" cellpadding="0" cellspacing="0">
        ...
        <tr>
          <td style="padding:28px 28px 20px;">
            <p style="margin:0;font-size:14px;color:#1e293b;line-height:1.6;">
              Bonjour <strong>${firstName} ${lastName}</strong>,<br/>
              Le loto <strong>${sessionName}</strong> organisé par <strong>${associationName}</strong> a lieu demain. Préparez vos cartons !
            </p>
          </td>
        </tr>
